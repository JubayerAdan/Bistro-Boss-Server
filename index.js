...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.bx1z9cc.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.post("/payments", async (req, res) => {
      ...
      const query = { _id: payment.cartIds.map((id) => new ObjectId(id)) };
      ...
      const pipeline = [
        ...
        },
        {
          $group: {
            ...
            items: { $push: "$menuItemsData.name" }, // Optional: Collect item names for debugging
          },
        },
        {
          $project: {
            ...
            count: 1,
            total: { $round: ["$total", 2] },
            items: 1, // Optional: Include item names in the result
            _id: 0,
          },
        },
      ];
